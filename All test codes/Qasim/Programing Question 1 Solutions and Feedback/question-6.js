...
function match_duplicate() {
  ...
  const FirstResultOFQ = document.getElementById("FirstResultOFQ-6");
  if (arr1[0] === arr2[0] && arr1[arr1.length - 1] === arr2[arr2.length - 1]) {
    countMatch = +4;
    FirstResultOFQ.innerHTML = "True both first and last elements are equal";
  } else if (arr1[0] === arr2[0]) {
    countMatch = +2;
    FirstResultOFQ.innerHTML = "True only first element is matched";
  } else if (arr1[arr1.length - 1] === arr2[arr2.length - 1]) {
    countMatch = +2;
    FirstResultOFQ.innerHTML = "True only last element is matched";
  } else {
    FirstResultOFQ.innerHTML = "condition is not met";
  }
  document.getElementById("SecondResultOFQ-6").innerHTML =
    "The count of matching elements are" + " " + countMatch;

  if (countMatch >= arr1.length + arr2.length) {
    finalNewArr = arr1;
    arr1 = arr2;
    arr2 = finalNewArr;
    document.getElementById(
      "ThirdResultOFQ-6"
    ).innerHTML = `new arrays wil be [ ${arr1} ],[ ${arr2} ]`;
  }
}
